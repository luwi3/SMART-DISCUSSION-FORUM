   ...
   <div class="top-brand-bar">
    ...
    <div id="notification-dropdown" style="position: relative; display: inline-block;">
        <a href="{{ route('student.dashboard', ['tab' => 'notifications']) }}" style="background: none; border: none; cursor: pointer; padding: 4px; display: flex; align-items: center; position: relative;">
            ...
            <span id="notification-count" style="position: absolute; top: -2px; right: -2px; background: #ef4444; color: white; border-radius: 9999px; padding: 2px 6px; font-size: 9px; font-weight: bold; line-height: 1; display: {{ (isset($initialUnread) && $initialUnread > 0) ? 'inline-block' : 'none' }};">
                {{ $initialUnread ?? 0 }}
            </span>
        </a>
    </div>
        ...
        <ul class="sidebar-menu">
            ...
            <li class="menu-item {{ (isset($currentTab) && $currentTab === 'notifications') ? 'active' : '' }}">
                <a href="{{ route('student.dashboard', ['tab' => 'notifications']) }}">
                    Notifications
                    <span class="badge" id="sidebar-badge-counter" style="display: {{ (isset($initialUnread) && $initialUnread > 0) ? 'inline-block' : 'none' }};">{{ $initialUnread ?? 0 }}</span>
                </a>
            </li>
...
<div class="right-column">
    <section class="content-panel">
        ...
        <div class="feed-list">
            @php
                $displayAnnouncements = (isset($announcements) && !is_null($announcements) && $announcements->count() > 0) 
                    ? $announcements->take(3) 
                    : \App\Models\Announcement::latest()->take(3)->get();
            @endphp

            @forelse($displayAnnouncements as $announcement)
                <div class="feed-item">
                    <div class="feed-avatar">
                        {{ strtoupper(substr($announcement->courseCode ?? $announcement->course_code ?? '📢', 0, 1)) }}
                    </div>
                    <div>
                        <div class="feed-msg-title" style="font-weight: 600; color: #0f172a;">
                            {{ $announcement->title }}
                        </div>
                        ...
                        <div class="feed-time" style="font-size: 11px; color: #94a3b8; margin-top: 4px;">
                            {{ $announcement->created_at ? $announcement->created_at->diffForHumans() : 'Recently' }}
                        </div>
                    </div>
                </div>
            @empty
                <p style="color: #64748b; font-size: 13px; padding: 10px 0;">No announcements posted yet.</p>
            @endforelse
        </div>
